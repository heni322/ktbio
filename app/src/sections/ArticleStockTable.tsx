  ...
  const navBtn = (disabled: boolean, onClick: () => void, icon: React.ReactNode) => (
    <button disabled={disabled} onClick={onClick}
      className="h-7 w-7 flex items-center justify-center rounded-md text-gray-500
                 hover:bg-teal-50 hover:text-teal-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
      {icon}
    </button>
  );
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-2.5 bg-white border-t border-gray-100">
      <div className="flex items-center gap-3">
        <span className="text-sm text-gray-500">
          <b className="text-gray-800">{start}–{end}</b> sur <b className="text-gray-800">{totalCount}</b> articles
        </span>
      ...
      <div className="flex items-center gap-0.5">
        {navBtn(page === 1, () => onPage(1), <ChevronsLeft className="h-3.5 w-3.5" />)}
        {navBtn(page === 1, () => onPage(page - 1), <ChevronLeft className="h-3.5 w-3.5" />)}
        {pages.map((p, i) =>
          p === '…'
            ...
            : <button key={p} onClick={() => onPage(p as number)}
                className={`h-7 w-7 flex items-center justify-center rounded-md text-xs font-medium transition-all
                  ${page === p ? 'bg-teal-500 text-white shadow-sm shadow-teal-200' : 'text-gray-600 hover:bg-teal-50 hover:text-teal-600'}`}>
                {p}
              </button>
        )}
        {navBtn(page >= totalPages || totalPages === 0, () => onPage(page + 1), <ChevronRight className="h-3.5 w-3.5" />)}
        {navBtn(page >= totalPages || totalPages === 0, () => onPage(totalPages), <ChevronsRight className="h-3.5 w-3.5" />)}
      </div>
    </div>
  );
}
...
function StatsBar({ total, pageTotal, critical, pageCount }: {
  ...
  return (
    <div className="grid grid-cols-4 gap-2 px-4 py-2.5 bg-gradient-to-r from-slate-50 to-teal-50/20 border-b border-gray-100">
      ...
      {stat(<span className="text-xs font-extrabold text-red-600">!</span>, 'Critiques (page)',
        <span className={critical > 0 ? 'text-red-600' : ''}>{critical}</span>, 'bg-red-100')}
    </div>
  );
}
...
export function ArticleStockTable() {
  ...
  const buildRequest = useCallback((p: number, ps: number, s: string, fam: string): FilterRequest => ({
  ...
  const load = useCallback(async (p: number, ps: number, s: string, fam: string, silent = false) => {
  ...
  useEffect(() => { load(page, pageSize, search, famille); }, [page, pageSize, search, famille]); // eslint-disable-line
  ...
  const updateLocal = (articleRef: string, id: number, delta: number) => {
    setResp(prev => prev ? ({
      ...prev,
      articles: prev.articles.map(art => art.arRef !== articleRef ? art : {
        ...art,
        total: art.total + delta,
        depots: art.depots.map(dep => ({
          ...dep,
          totalQte: dep.lots.some(l => l.id === id) ? dep.totalQte + delta : dep.totalQte,
          lots: dep.lots.map(l => l.id === id ? { ...l, quantite: Math.max(0, l.quantite + delta) } : l),
        })),
      }),
    }) : prev);
  };
      ...
      <div className="h-full flex flex-col bg-gray-50">
        <div className="p-4 bg-white border-b border-gray-100 space-y-3">
          ...
          <div className="flex gap-3">
            {[52, 36, 32].map(w => <div key={w} className="h-9 bg-gray-100 rounded-lg animate-pulse" style={{ width: w * 4 }} />)}
          </div>
        </div>
        <div className="flex-1 p-4 space-y-2">
          {[...Array(10)].map((_, i) => (
            <div key={i} className="h-12 bg-white rounded-xl border border-gray-100 animate-pulse" style={{ opacity: 1 - i * 0.08 }} />
          ))}
        </div>
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm">
        <div className="px-4 pt-3.5 pb-2.5 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => load(page, pageSize, search, famille, true)} disabled={refreshing}
              className="h-8 w-8 flex items-center justify-center rounded-lg border border-gray-200
                         text-gray-400 hover:border-teal-300 hover:text-teal-600 hover:bg-teal-50 transition-all disabled:opacity-50"
              title="Actualiser">
              <RefreshCw className={`h-3.5 w-3.5 ${refreshing ? 'animate-spin' : ''}`} />
            </button>
            <Button className="h-8 bg-teal-500 hover:bg-teal-600 text-white text-xs px-3 shadow-sm shadow-teal-200"
              onClick={handleExcelExport}>
              <FileSpreadsheet className="h-3.5 w-3.5 mr-1.5" />Exporter
            </Button>
          </div>
        </div>
        ...
        <div className="px-4 pb-2.5 flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Select value={viewMode} onValueChange={v => setViewMode(v as typeof viewMode)}>
              <SelectTrigger className="h-8 w-32 text-xs border-gray-200 bg-gray-50"><SelectValue /></SelectTrigger>
              <SelectContent>
      ...
      {/* ── Stats ── */}
      <StatsBar total={totalCount} pageCount={articles.length} pageTotal={pageTotal} critical={critical} />
      ...
      <div className="flex-1 overflow-auto relative">
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {articles.length === 0 ? (
              <tr>
                <td colSpan={depots.length + 6} className="py-20 text-center">
              ...
              return (
                <tr key={art.arRef}
                  ...
                  {isNewLongueur ? (() => {
                    ...
                    return (
                      <td rowSpan={span}
                        className="px-3 py-2.5 text-center font-bold text-teal-700 bg-teal-50 sticky left-0 z-20 border-r border-teal-200/40 min-w-[80px] align-middle text-sm">
                        {art.longueur != null ? art.longueur : <span className="text-gray-300">—</span>}
                      </td>
                    );
                  })() : null}
                              ...
                              {viewMode === 'Lot' ? (
                                ...
                                <div className={`rounded-lg border px-2 py-1 ${getCriticalBorder(lot.criticalPeriodMonths)}`}>
                                  ...
                                  <div className="flex items-center justify-center gap-1.5">
                                    ...
                                    <span className="text-xs font-bold text-gray-900 w-5 text-center tabular-nums">{lot.quantite}</span>
                                    <button
          ...
          {articles.length > 0 && (
            <tfoot className="sticky bottom-0 z-20">
              <tr className="bg-gradient-to-r from-teal-50 to-teal-100/50 border-t-2 border-teal-200">
                <td className="px-3 py-2 font-bold text-xs sticky left-0 bg-teal-50 text-teal-700" colSpan={5}>
                  Sous-total ({articles.length} articles)
      ...
      {/* ── Pagination ── */}
      <Pagination
        page={page} totalPages={totalPages} pageSize={pageSize} totalCount={totalCount}
